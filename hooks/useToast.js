...
// ไอคอนสำหรับ toast แต่ละประเภท (คงเดิม)
const toastIcons = {
  success: (
...
export default function useToast() {
  ...
  const success = useCallback((message, options = {}) => {
    return toast.success(message, {
      ...
      // ลบ style ออก เพื่อให้ใช้ CSS จาก globals.css
    });
  }, []);
  ...
  const dismiss = useCallback((toastId) => {
    if (toastId) {
      ...
      toast.dismiss(); // ปิดทุก toast ถ้าไม่ระบุ ID
    }
  }, []);

  // ส่งคืนชุดฟังก์ชันสำหรับใช้งาน
  return {
    success,
    error,
    warning,
